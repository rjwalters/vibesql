...
error-no-databases = Keine Datenbanken verfügbar

# Loading states
loading-initializing-theme = Theme wird initialisiert
loading-preparing-editor = Editor wird vorbereitet
loading-database-engine = Datenbankmodul wird geladen
loading-setting-up-ui = Benutzeroberfläche wird eingerichtet
loading-editor = Editor wird geladen...
loading-compliance-data = Compliance-Daten werden geladen...
loading-conformance-report = Konformitätsbericht wird geladen...
...
bench-no-server-data = Keine Sysbench-Server-Benchmark-Daten verfügbar
bench-no-server-data-hint = Server-Benchmarks erfordern die Ausführung von sysbench_server mit aktiviertem MySQL-Vergleich.
...
bench-sysbench-emb-disc-point-title = Punktabfragen: { $pointRatio } Abstand
bench-sysbench-emb-disc-point = VibeSQLs Punktabfragen laufen bei <strong>~{ $pointVibesqlUs }µs vs SQLites ~{ $pointSqliteUs }µs</strong>. Dieser { $pointRatio } Abstand ist unser primäres OLTP-Optimierungsziel - wir untersuchen B-Tree-Knotenlayout und sperrenfreie Lesepfade, um diesen Abstand zu schließen.
bench-sysbench-emb-disc-index-title = Index-Updates: { $indexRatio } Abstand
bench-sysbench-emb-disc-index = VibeSQLs indizierte Updates laufen bei <strong>~{ $indexVibesqlUs }µs vs SQLites ~{ $indexSqliteUs }µs</strong>. Dies ist ein Optimierungsbereich, da unser MVCC-Design Overhead für Index-Wartung hinzufügt, den wir reduzieren möchten.
...
bench-sysbench-emb-disc-nonindex = Nicht-indizierte Updates zeigen VibeSQL bei ~{ $nonIndexVibesqlUs }µs vs SQLites ~{ $nonIndexSqliteUs }µs
bench-sysbench-emb-disc-deletes = Löschoperationen zeigen VibeSQL bei ~{ $deleteVibesqlUs }µs vs SQLites ~{ $deleteSqliteUs }µs
...
bench-sysbench-srv-disc-mysql = Server-Benchmarks vergleichen mit MySQL, um VibeSQL als Drop-in-Ersatz für traditionelle Client-Server-Datenbanken zu bewerten. VibeSQL Server übertrifft MySQL bei allen Sysbench-Operationen mit Beschleunigungen von <strong>2,4x</strong> (Range-Abfragen) bis <strong>12,8x</strong> (indizierte Updates).
bench-sysbench-srv-disc-perf-title = Warum VibeSQL Server schneller ist
bench-sysbench-srv-disc-perf-arch = Die Architektur von VibeSQL unterscheidet sich grundlegend vom traditionellen RDBMS-Design von MySQL
bench-sysbench-srv-disc-perf-storage = VibeSQL verwendet eine spaltenorientierte In-Memory-Speicher-Engine, optimiert für analytische und OLTP-Workloads, und vermeidet den Overhead der festplattenbasierten InnoDB-Seitenverwaltung von MySQL
bench-sysbench-srv-disc-perf-locking = Keine aufwendige Zeilensperrung oder MVCC-Buchführung—VibeSQL verwendet leichtgewichtige Nebenläufigkeitssteuerung für moderne Mehrkern-CPUs
bench-sysbench-srv-disc-perf-protocol = Effiziente PostgreSQL-Protokollimplementierung mit minimalem Serialisierungs-Overhead im Vergleich zum MySQL-Protokoll
bench-sysbench-srv-disc-perf-writes = Schreiboperationen (Einfügungen/Updates) zeigen die größten Gewinne (<strong>8-12x</strong>), da VibeSQL MySQLs Redo-Log, Undo-Log und Doublewrite-Buffer-Synchronisation vermeidet
bench-sysbench-srv-disc-perf-reads = Leseoperationen zeigen kleinere aber konsistente Gewinne (<strong>2-3x</strong>) durch cache-effiziente spaltenorientierte Zugriffsmuster und kein Disk-I/O
...
# TPC-H Server spezifisch
bench-tpch-server-name = TPC-H (Server)
bench-tpch-server-title = TPC-H Analytischer Benchmark (Server)
bench-tpch-server-description = <strong>TPC-H Server-Benchmarks</strong> vergleichen VibeSQL Server (PostgreSQL-Protokoll) mit MySQL für analytische Abfrage-Workloads und messen die OLAP-Leistung in Client-Server-Bereitstellungen.
bench-tpch-server-ops-label = TPC-H Abfragen
bench-tpch-server-note-intro = Server-Benchmarks testen die <strong>PostgreSQL-Protokoll</strong>-Implementierung und messen die End-to-End-Abfragelatenz einschließlich Netzwerk-Overhead.
bench-tpch-server-note-queries = Abfragen testen komplexe JOINs, Unterabfragen und Aggregationen, die typisch für Business-Intelligence-Workloads sind.

# TPC-H Server Diskussion
bench-tpch-srv-disc-protocol-title = PostgreSQL-Protokoll
bench-tpch-srv-disc-protocol = VibeSQL Server spricht das PostgreSQL-Protokoll und ermöglicht die Verwendung von Standard-PostgreSQL-Treibern und -Tools. Dieser Benchmark misst die vollständige End-to-End-Latenz einschließlich Protokoll-Overhead.
bench-tpch-srv-disc-comparison-title = MySQL-Vergleich
bench-tpch-srv-disc-comparison = Der Vergleich mit MySQL bietet eine Baseline für traditionelle Client-Server-Datenbanken bei analytischen Workloads. Die spaltenorientierte Ausführungs-Engine von VibeSQL bietet Vorteile für komplexe Aggregationen und Joins.
bench-tpch-srv-disc-roadmap-title = Server OLAP Roadmap
bench-tpch-srv-disc-prepared = Wiederverwendung kompilierter Abfragepläne über Verbindungen hinweg
bench-tpch-srv-disc-pooling = Effiziente Verbindungsbehandlung für Hochdurchsatzszenarien
bench-tpch-srv-disc-scale = Tests größerer Datensätze (SF 0.1, SF 1.0) zur Validierung im Produktionsmaßstab

# TPC-C Server spezifisch
bench-tpcc-server-name = TPC-C (Server)
bench-tpcc-server-title = TPC-C OLTP Benchmark (Server)
bench-tpcc-server-description = <strong>TPC-C Server-Benchmarks</strong> vergleichen VibeSQL Server (PostgreSQL-Protokoll) mit MySQL für OLTP-Transaktions-Workloads und messen den Durchsatz für Multi-Client-Datenbankbereitstellungen.
bench-tpcc-server-ops-label = TPC-C Transaktionen
bench-tpcc-server-note-intro = Server-Benchmarks testen die <strong>PostgreSQL-Protokoll</strong>-Implementierung und messen den Transaktionsdurchsatz einschließlich Netzwerk-Overhead.
bench-tpcc-server-note-results = Ergebnisse berichten Transaktionen pro Sekunde (TPS) für den Standard-TPC-C-Transaktionsmix.
bench-tpcc-mixed = Gemischte Arbeitslast - Standard-TPC-C-Transaktionsmix (45% Neue-Bestellung, 43% Zahlung, 4% Bestellstatus, 4% Lieferung, 4% Lagerbestand)

# TPC-C Server Diskussion
bench-tpcc-srv-disc-protocol-title = PostgreSQL-Protokoll
bench-tpcc-srv-disc-protocol = VibeSQL Server spricht das PostgreSQL-Protokoll und ermöglicht die Verwendung von Standard-PostgreSQL-Treibern und -Tools. Dieser Benchmark misst die vollständige End-to-End-Transaktionslatenz einschließlich Protokoll-Overhead.
bench-tpcc-srv-disc-comparison-title = MySQL-Vergleich
bench-tpcc-srv-disc-comparison = Der Vergleich mit MySQL bietet eine Baseline für traditionelle Client-Server-Datenbanken bei OLTP-Workloads. MySQL ist der Industriestandard für transaktionale Workloads, und TPC-C ist die Stärke von MySQL.
bench-tpcc-srv-disc-roadmap-title = Server OLTP Roadmap
bench-tpcc-srv-disc-prepared = Wiederverwendung kompilierter Abfragepläne über Verbindungen hinweg
bench-tpcc-srv-disc-pooling = Effiziente Verbindungsbehandlung für Hochdurchsatzszenarien
bench-tpcc-srv-disc-parallel = Gleichzeitige Multi-Client-Transaktionsverarbeitung
...
bench-bullet-concurrency = Leichtgewichtige Nebenläufigkeit
bench-bullet-protocol = Protokolleffizienz
bench-bullet-writes = Schreiboperationen
bench-bullet-reads = Leseoperationen
...
bench-bullet-prepared-stmts = Vorbereitete Anweisungen
bench-bullet-larger-scale = Größere Skalierungsfaktoren
bench-bullet-parallel-clients = Parallele Clients
...
# SQLite TCL-Testsuite-Abschnitt
conformance-tcl-title = SQLite TCL-Testsuite
conformance-tcl-desc = Ergebnisse der kanonischen <a href="https://www.sqlite.org/testing.html" target="_blank" class="text-blue-600 dark:text-blue-400 hover:underline">TCL-Testsuite</a> von SQLite mit { $fileCount } Testdateien. Diese Suite ist der Goldstandard für SQLite-Kompatibilitätstests.
conformance-tcl-overall-rate = Gesamterfolgsrate
conformance-tcl-tests-passing = { $passed } von { $total } Tests bestanden
conformance-tcl-passed = Bestanden
conformance-tcl-failed = Fehlgeschlagen
conformance-tcl-skipped = Übersprungen
conformance-tcl-total = Gesamt
conformance-tcl-categories-title = Testkategorien
conformance-tcl-category = Kategorie
conformance-tcl-rate = Rate
conformance-tcl-progress = Fortschritt
conformance-tcl-tests = Tests
conformance-tcl-common-failures = Häufige Fehler
conformance-tcl-failure-patterns = Top { $count } Fehlermuster nach Häufigkeit
conformance-tcl-about-title = Über TCL-Tests:
conformance-tcl-about-text = Die TCL-Testsuite von SQLite ist der kanonische Konformitätstest für SQLite-Kompatibilität. Sie testet spezifische SQLite-Verhaltensweisen, Eigenheiten und Grenzfälle, die von Standard-SQL-Testsuites möglicherweise nicht abgedeckt werden. Hohe Erfolgsraten hier zeigen starke SQLite-Kompatibilität für Anwendungsmigrationsszenarien an.
...
# Seitentitel und Kopfzeile
challenge-page-title = SQL Vibe Coding Challenge - VibeSQL
challenge-header = SQL Vibe Coding Challenge

# Hero-Bereich
challenge-hero-title = Die SQL Vibe Coding Challenge
challenge-hero-subtitle = Ein objektiver Benchmark für Multi-Agenten-Softwareentwicklung. Baue eine SQL-Datenbank von Grund auf. Bestehe 6 Millionen Tests. Gewinne die Trophäe.
challenge-btn-start = Mit dem Bauen beginnen
challenge-btn-trophy = Trophäe ansehen
challenge-btn-leaderboard = Bestenliste

# Wichtige Erkenntnis
challenge-insight-title = Die einzige Metrik, die zählt: Kalenderzeit
challenge-insight-text = Commits und Codezeilen sind nur Proxy-Metriken. Was zählt, sind <strong>Tage bis zur Fertigstellung</strong>. Können 1.000 parallel arbeitende Agenten 100 Agenten schlagen? Behält dein Orchestrierungs-Framework die Produktivität bei Skalierung bei? Dieser Benchmark wird es dir verraten.

# Challenge-Bereich
challenge-section-title = Die Challenge
challenge-objective-title = Ziel
challenge-objective-text = Baue eine SQL-Datenbank-Engine von Grund auf, die die <a href="https://www.sqlite.org/sqllogictest/" class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank" rel="noopener">SQLLogicTest-Suite</a> besteht. Dies ist dieselbe Testsuite, die zur Validierung von SQLite, DuckDB und anderen Produktionsdatenbanken verwendet wird.
challenge-success-title = Erfolgskriterien
challenge-success-pass-rate = 100% Bestehensquote bei SQLLogicTest
challenge-success-assertions = ~6 Millionen einzelne Test-Assertions
challenge-success-files = Alle 622 Testdateien bestanden
challenge-constraints-title = Einschränkungen
challenge-constraint-parser = <strong>Keine bestehenden SQL-Parser-Bibliotheken</strong> — baue deinen eigenen Parser
challenge-constraint-engine = <strong>Keine bestehenden Query-Engines</strong> — implementiere die Ausführung von Grund auf
challenge-constraint-libs = <strong>Keine datenbankspezifischen Bibliotheken</strong> — verwende nur Allzweckbibliotheken
challenge-allowed-title = Erlaubt
challenge-allowed-lang = Jede Programmiersprache
challenge-allowed-ai = Jedes AI-Orchestrierungs-Framework
challenge-allowed-human = Menschliche Intervention (unbegrenzt)
challenge-allowed-libs = Allzweckbibliotheken (Datenstrukturen, I/O, etc.)

# Trophäen-Bereich
challenge-trophy-title = Die Trophäe
challenge-trophy-name = Die Vibe Coding Trophäe
challenge-trophy-desc = Eine physische Trophäe wird jedem Rekordhalter verliehen. Das Design spiegelt den Geist des „Vibe Coding" wider — ein vergoldeter Stab auf Nussbaumholz mit Messing-Namensschildern.
challenge-trophy-claim = <strong>Dein Name kommt auf die Trophäe</strong>, wenn du den aktuellen Rekord um mindestens 5% übertriffst.
challenge-rules-title = Preisregeln
challenge-rule-improve = <strong>5% Verbesserung erforderlich</strong> — übertreffe den vorherigen Rekord um mindestens 5% (in Kalendertagen), um die Trophäe zu beanspruchen
challenge-rule-public = <strong>Öffentliches Repository</strong> — dein Code muss öffentlich zur Überprüfung verfügbar sein
challenge-rule-pass = <strong>100% Bestehensquote</strong> — alle 622 SQLLogicTest-Dateien müssen bestanden werden
challenge-rule-git = <strong>Überprüfbare Git-Historie</strong> — das Datum des ersten Commits bis zur 100% Bestehensquote bestimmt deine Zeit
challenge-record-title = Aktueller Rekordhalter
challenge-record-days = { $days } Tage
challenge-record-name = VibeSQL (Baseline)
challenge-record-date = Oktober - November 2025
challenge-record-beat = Das um 5% übertreffen? Das bedeutet <strong>{ $target } Tage oder weniger</strong>, um die Trophäe zu beanspruchen.

# Warum diese Challenge-Bereich
challenge-why-title = Warum diese Challenge?
challenge-why-objective-title = Objektive Messung
challenge-why-objective-text = Keine subjektiven Code-Reviews. Tests bestehen oder bestehen nicht. 6 Millionen Assertions lassen keinen Raum für Mehrdeutigkeit.
challenge-why-complexity-title = Echte Komplexität
challenge-why-complexity-text = SQL-Datenbanken erfordern Parser, Optimierer und Ausführungs-Engines. Dies ist kein Spielzeugproblem — es ist produktionsreifes Engineering.
challenge-why-time-title = Zeit ist Wahrheit
challenge-why-time-text = Kalendertage bis zur Fertigstellung ist die ultimative Metrik. Hilft die Parallelisierung auf 1.000 Agenten? Jetzt kannst du es herausfinden.

# Start-Bereich
challenge-start-title = Loslegen
challenge-start-intro = Starte von Grund auf in jeder Sprache, oder nutze eines unserer Seed-Repositories für Bequemlichkeit. Jedes Seed enthält die SQLLogicTest-Suite, einen Test-Runner und einen CI-Workflow.
challenge-seed-title = Seed-Repositories
challenge-seed-optional = (optional)
challenge-seed-rust-desc = Cargo-Buildsystem, Zero-Cost-Abstraktionen, Speichersicherheit ohne GC.
challenge-seed-cpp-desc = CMake-Buildsystem, maximale Leistung, volle Kontrolle über den Speicher.
challenge-seed-go-desc = Einfache Toolchain, schnelle Kompilierung, ausgezeichnete Nebenläufigkeits-Primitive.
challenge-seed-fork = Fork auf GitHub →
challenge-step1-title = Starte dein Projekt
challenge-step1-text = Erstelle ein neues Repo von Grund auf, oder forke ein Seed oben für einen schnellen Start. Hole dir die <a href="https://www.sqlite.org/sqllogictest/" class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank" rel="noopener">SQLLogicTest-Suite</a>. Dein erster Commit startet die Uhr.
challenge-step2-title = Baue deine Datenbank
challenge-step2-text = Implementiere einen SQL-Parser, Query-Executor und Storage-Engine. Nutze beliebige AI-Tools — Claude, Copilot oder deine eigenen Agenten. Führe <code class="bg-gray-200 dark:bg-gray-700 px-1 rounded">make test</code> aus, um den Fortschritt zu verfolgen.
challenge-step3-title = Erreiche 100% und reiche ein
challenge-step3-text = Wenn alle 622 Testdateien bestanden sind, eröffne ein Issue bei <a href="https://github.com/vibesql-challenge/submissions" class="text-blue-600 dark:text-blue-400 hover:underline" target="_blank" rel="noopener">vibesql-challenge/submissions</a> mit deinem Repo-Link und Commit-Hashes. Unterbiete 25 Tage, um der Bestenliste beizutreten.

# VibeSQL erkunden-Bereich
challenge-explore-title = VibeSQL erkunden
challenge-explore-demo-title = Demo ausprobieren
challenge-explore-demo-text = Führe SQL-Abfragen in deinem Browser mit dem WebAssembly-Build aus.
challenge-explore-conformance-title = Konformitätsbericht
challenge-explore-conformance-text = Detaillierte Aufschlüsselung der SQL:1999-Standardkonformität.
challenge-explore-benchmarks-title = Leistungs-Benchmarks
challenge-explore-benchmarks-text = TPC-H, TPC-C und andere Benchmarks im Vergleich zu SQLite und DuckDB.

# Fußzeile
challenge-footer = VibeSQL - SQL:1999-Datenbank in WebAssembly
