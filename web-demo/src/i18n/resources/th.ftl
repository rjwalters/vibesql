...
bench-tpcc-disc-faster = VibeSQL บรรลุ <strong>ประมาณ 23,000 ธุรกรรมต่อวินาที</strong> เปรียบเทียบกับ ~{ $sqliteTps } TPS ของ SQLite เป็นการปรับปรุง 5 เท่า ความเร็วนี้มาจากสถาปัตยกรรม MVCC แบบไม่ล็อกของเราที่หลีกเลี่ยงการล็อกแบบหยาบของ SQLite ในทุกการดำเนินการเขียน
...
bench-sysbench-srv-disc-mysql = เบนช์มาร์กเซิร์ฟเวอร์เปรียบเทียบกับ MySQL เพื่อประเมิน VibeSQL เป็นตัวแทนที่ดรอปอินสำหรับฐานข้อมูลไคลเอนต์-เซิร์ฟเวอร์แบบดั้งเดิม VibeSQL Server เอาชนะ MySQL ในทุกการดำเนินการ Sysbench ด้วยความเร็วเพิ่มขึ้นจาก <strong>2.4x</strong> (คิวรีช่วง) ถึง <strong>12.8x</strong> (อัปเดตที่มีดัชนี)
bench-sysbench-srv-disc-perf-title = ทำไม VibeSQL Server ถึงเร็วกว่า
bench-sysbench-srv-disc-perf-arch = สถาปัตยกรรมของ VibeSQL แตกต่างอย่างพื้นฐานจากการออกแบบ RDBMS แบบดั้งเดิมของ MySQL
bench-sysbench-srv-disc-perf-storage = VibeSQL ใช้เครื่องยนต์จัดเก็บแบบคอลัมน์ในหน่วยความจำที่ปรับให้เหมาะสมสำหรับภาระงานวิเคราะห์และ OLTP หลีกเลี่ยงค่าใช้จ่ายการจัดการหน้า InnoDB บนดิสก์ของ MySQL
bench-sysbench-srv-disc-perf-locking = ไม่มีการล็อคระดับแถวที่หนักหรือการบัญชี MVCC—VibeSQL ใช้การควบคุมการทำงานพร้อมกันแบบเบาที่ออกแบบสำหรับ CPU หลายแกนสมัยใหม่
bench-sysbench-srv-disc-perf-protocol = การใช้งานโปรโตคอลสาย PostgreSQL ที่มีประสิทธิภาพพร้อมค่าใช้จ่ายการซีเรียลไลซ์น้อยที่สุดเมื่อเทียบกับโปรโตคอล MySQL
bench-sysbench-srv-disc-perf-writes = การดำเนินการเขียน (แทรก/อัปเดต) แสดงการเพิ่มขึ้นมากที่สุด (<strong>8-12x</strong>) เพราะ VibeSQL หลีกเลี่ยงการซิงค์ redo log, undo log และ doublewrite buffer ของ MySQL
bench-sysbench-srv-disc-perf-reads = การดำเนินการอ่านแสดงการเพิ่มขึ้นน้อยกว่าแต่สม่ำเสมอ (<strong>2-3x</strong>) เนื่องจากรูปแบบการเข้าถึงคอลัมน์ที่มีประสิทธิภาพแคชและไม่มี I/O ดิสก์
...
bench-footprint-emb-disc-size = VibeSQL ที่ <strong>~{ $vibesqlBinaryMb }MB</strong> อยู่ระหว่าง SQLite (~{ $sqliteBinaryMb }MB) และ DuckDB (~{ $duckdbBinaryMb }MB) สิ่งนี้สะท้อนถึงการเลือกของเราที่จะรวมคุณสมบัติขั้นสูง (ฟังก์ชันหน้าต่าง, CTE, การดำเนินการแบบคอลัมน์) ในขณะที่รักษาไบนารีให้จัดการได้สำหรับการติดตั้งแบบฝังตัว
bench-footprint-emb-disc-startup-title = การเริ่มต้น: การเริ่มต้นเย็นที่เร็วที่สุด
bench-footprint-emb-disc-startup = VibeSQL บรรลุ <strong>การเริ่มต้นเย็น ~6ms</strong> เร็วกว่า SQLite (~{ $sqliteStartupMs }ms) และเร็วกว่า DuckDB (~{ $duckdbStartupMs }ms) อย่างมาก เส้นทางการเริ่มต้นขั้นต่ำของเราโหลดเฉพาะโครงสร้างเมตาดาต้าที่จำเป็นเมื่อเริ่มต้น
...
bench-footprint-srv-disc-wasm = โมดูล WebAssembly ของ VibeSQL บีบอัดเป็น <strong>~{ $wasmSizeGzipMb }MB gzipped</strong> ทำให้สามารถโหลดหน้าเริ่มต้นได้อย่างรวดเร็ว นี่คือฐานข้อมูล SQL:1999 เต็มรูปแบบพร้อมฟังก์ชันหน้าต่าง, CTE และธุรกรรม ACID ที่ทำงานทั้งหมดในเบราว์เซอร์
...
home-page-title = VibeSQL — ฐานข้อมูล SQL ด้วย Rust ล้วนที่สร้างมาเพื่อความเร็ว

# ส่วน Hero
home-hero-title = ฐานข้อมูล SQL ด้วย Rust ล้วน<br>สร้างมาเพื่อความเร็ว
home-hero-subtitle = VibeSQL แลกเปลี่ยนประสิทธิภาพการจัดเก็บเพื่อประสิทธิภาพการทำงาน การจัดเก็บแบบผสม row + columnar, การดำเนินการแบบ vectorized, และโค้ด unsafe เป็นศูนย์ ปรับแต่งสำหรับชุดข้อมูลที่อยู่ในหน่วยความจำได้
home-hero-subtext = สอดคล้องกับ SQL:1999 ทำงานแบบ native, ใน WebAssembly, และเป็นไลบรารีแบบฝัง
home-btn-demo = ลองในเบราว์เซอร์
home-btn-github = GitHub
home-btn-crates = crates.io

# ส่วนทำไม VibeSQL
home-why-title = ทำไมต้อง VibeSQL?
home-hybrid-title = การจัดเก็บแบบผสม
home-hybrid-text = การจัดเก็บทั้งแบบ row และ columnar ในเอนจิ้นเดียว รูปแบบ row สำหรับการค้นหาจุดที่รวดเร็วและ OLTP รูปแบบ columnar สำหรับการสแกนวิเคราะห์พร้อมการดำเนินการแบบ vectorized ไม่ต้องเลือก
home-speed-title = ความเร็วเหนือการจัดเก็บ
home-speed-text = แลกเปลี่ยนพื้นที่ดิสก์โดยตั้งใจเพื่อประสิทธิภาพคิวรี เค้าโครงการจัดเก็บซ้ำซ้อน, การแคชเชิงรุก, และดัชนีที่คำนวณล่วงหน้าหมายความว่าฐานข้อมูลขนาดเล็กทำงานเร็วที่สุดเท่าที่จะเป็นไปได้
home-rust-title = Rust ล้วน, Unsafe เป็นศูนย์
home-rust-text = เขียนด้วย safe Rust ทั้งหมด ไม่มีการพึ่งพา C, ไม่มี FFI, ไม่มีบล็อก unsafe คอมไพล์เป็นไบนารี native และ WebAssembly จากโค้ดเบสเดียวกัน

# ส่วนสถาปัตยกรรม
home-arch-title = สถาปัตยกรรม
home-pipeline-title = ขั้นตอนการประมวลผลคิวรี
home-pipeline-parser = <strong>Parser</strong> — ไวยากรณ์ SQL:1999 เต็มรูปแบบ, AST แบบ arena-allocated
home-pipeline-planner = <strong>Planner</strong> — ตัวปรับแต่งแบบ cost-based พร้อมการจัดเรียง join ใหม่
home-pipeline-executor = <strong>Executor</strong> — การดำเนินการแบบ vectorized พร้อมการประมวลผลเป็นชุด
home-pipeline-storage = <strong>Storage</strong> — แบบผสม row/columnar พร้อมดัชนี B-tree
home-features-title = คุณสมบัติหลัก
home-feature-window = ฟังก์ชันหน้าต่าง (ROW_NUMBER, RANK, LEAD/LAG, NTILE, ...)
home-feature-cte = Common Table Expressions (WITH, CTEs แบบ recursive)
home-feature-subquery = Subqueries (correlated, EXISTS, IN, scalar)
home-feature-join = รองรับ JOIN เต็มรูปแบบ (INNER, LEFT, RIGHT, FULL, CROSS, NATURAL)
home-feature-triggers = Triggers, views, foreign keys, CHECK constraints
home-feature-wasm = เป้าหมาย WASM พร้อมการจัดเก็บถาวร OPFS

# ส่วนประสิทธิภาพ
home-perf-title = ประสิทธิภาพ
home-perf-full = ดูเกณฑ์มาตรฐานทั้งหมด →
home-stat-tpch-label = คิวรี TPC-H ที่ผ่าน
home-stat-tpch-sub = เกณฑ์มาตรฐานการสนับสนุนการตัดสินใจ
home-stat-conformance-label = อัตราการผ่าน SQLLogicTest
home-stat-conformance-sub = การยืนยันทดสอบกว่า 6 ล้านรายการ
home-stat-tpcds-label = คิวรี TPC-DS ที่ผ่าน
home-stat-tpcds-sub = เกณฑ์มาตรฐานการวิเคราะห์ซับซ้อน
home-perf-note = ทดสอบเทียบกับ SQLite, DuckDB และ MySQL บนภาระงานที่เทียบเท่า <a href="benchmarks.html" class="text-blue-600 dark:text-blue-400 hover:underline">ดูผลลัพธ์ทั้งหมด</a>

# ส่วนเริ่มต้น
home-start-title = เริ่มต้นใช้งาน
home-demo-title = สาธิตแบบโต้ตอบ
home-demo-text = รันคิวรี SQL ในเบราว์เซอร์ของคุณ เอนจิ้นฐานข้อมูลเต็มรูปแบบที่คอมไพล์เป็น WebAssembly พร้อมการจัดเก็บถาวรผ่าน OPFS ไม่ต้องติดตั้ง
home-install-title = ติดตั้ง
home-install-cargo = Cargo
home-install-library = เป็นไลบรารี

# ส่วนสำรวจ
home-explore-conformance-title = รายงานความสอดคล้อง
home-explore-conformance-text = การวิเคราะห์โดยละเอียดของการปฏิบัติตามมาตรฐาน SQL:1999 จาก 622 ไฟล์ทดสอบ
home-explore-bench-title = เกณฑ์มาตรฐานประสิทธิภาพ
home-explore-bench-text = ผลลัพธ์ TPC-H, TPC-DS, TPC-C และ Sysbench เทียบกับ SQLite, DuckDB และ MySQL

# ส่วนท้าย
home-footer = VibeSQL — ฐานข้อมูล SQL ด้วย Rust ล้วนที่สร้างมาเพื่อความเร็ว
