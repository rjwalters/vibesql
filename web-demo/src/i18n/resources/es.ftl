...
bench-sysbench-emb-disc-point-title = Búsquedas Puntuales: Brecha de { $pointRatio }
bench-sysbench-emb-disc-point = Las selecciones puntuales de VibeSQL ejecutan a <strong>~{ $pointVibesqlUs }µs vs ~{ $pointSqliteUs }µs de SQLite</strong>. Esta brecha de { $pointRatio } representa nuestro objetivo principal de optimización OLTP - estamos investigando el diseño de nodos B-tree y rutas de lectura sin bloqueo para cerrar esta brecha.
bench-sysbench-emb-disc-index-title = Actualizaciones de Índice: Brecha de { $indexRatio }
bench-sysbench-emb-disc-index = Las actualizaciones indexadas de VibeSQL ejecutan a <strong>~{ $indexVibesqlUs }µs vs ~{ $indexSqliteUs }µs de SQLite</strong>. Esta es un área de optimización ya que nuestro diseño MVCC agrega sobrecarga para el mantenimiento de índices que estamos trabajando en reducir.
...
bench-sysbench-emb-disc-nonindex = Las actualizaciones sin índice muestran VibeSQL a ~{ $nonIndexVibesqlUs }µs vs ~{ $nonIndexSqliteUs }µs de SQLite
bench-sysbench-emb-disc-deletes = Las operaciones de eliminación muestran VibeSQL a ~{ $deleteVibesqlUs }µs vs ~{ $deleteSqliteUs }µs de SQLite
bench-sysbench-emb-disc-architecture-title = Compensaciones Arquitectónicas
bench-sysbench-emb-disc-architecture = La arquitectura híbrida de VibeSQL apunta tanto a cargas OLTP como OLAP. Nuestro almacenamiento B-tree proporciona rendimiento de búsqueda puntual competitivo con SQLite, mientras que la ejecución columnar maneja consultas analíticas eficientemente. Esto difiere de bases de datos OLAP puras como DuckDB que optimizan exclusivamente para operaciones en masa a costa de la latencia de una sola fila.
...
bench-sysbench-srv-disc-mysql = Los benchmarks de servidor comparan contra MySQL para evaluar VibeSQL como reemplazo directo para bases de datos cliente-servidor tradicionales. VibeSQL Server supera a MySQL en todas las operaciones de Sysbench, con aceleraciones que van desde <strong>2,4x</strong> (consultas de rango) hasta <strong>12,8x</strong> (actualizaciones indexadas).
bench-sysbench-srv-disc-perf-title = Por qué VibeSQL Server es más rápido
bench-sysbench-srv-disc-perf-arch = La arquitectura de VibeSQL difiere fundamentalmente del diseño RDBMS tradicional de MySQL
bench-sysbench-srv-disc-perf-storage = VibeSQL usa un motor de almacenamiento columnar en memoria optimizado para cargas analíticas y OLTP, evitando la sobrecarga de gestión de páginas InnoDB basada en disco de MySQL
bench-sysbench-srv-disc-perf-locking = Sin bloqueo pesado a nivel de fila ni contabilidad MVCC—VibeSQL usa control de concurrencia ligero diseñado para CPUs multinúcleo modernas
bench-sysbench-srv-disc-perf-protocol = Implementación eficiente del protocolo PostgreSQL con sobrecarga de serialización mínima comparada con el protocolo de MySQL
bench-sysbench-srv-disc-perf-writes = Las operaciones de escritura (inserciones/actualizaciones) muestran las mayores ganancias (<strong>8-12x</strong>) porque VibeSQL evita la sincronización del redo log, undo log y doublewrite buffer de MySQL
bench-sysbench-srv-disc-perf-reads = Las operaciones de lectura muestran ganancias menores pero consistentes (<strong>2-3x</strong>) debido a patrones de acceso columnar eficientes en caché y cero E/S de disco
...
bench-bullet-concurrency = Concurrencia ligera
bench-bullet-protocol = Eficiencia del protocolo
bench-bullet-writes = Operaciones de escritura
bench-bullet-reads = Operaciones de lectura
