...
bench-sysbench-srv-disc-mysql = サーバーベンチマークは、従来のクライアントサーバーデータベースの代替としてVibeSQLを評価するためにMySQLと比較します。VibeSQL Serverはすべてのsysbench操作でMySQLを上回り、<strong>2.4x</strong>（範囲クエリ）から<strong>12.8x</strong>（インデックス付き更新）の高速化を達成しています。
bench-sysbench-srv-disc-perf-title = VibeSQL Serverが高速な理由
bench-sysbench-srv-disc-perf-arch = VibeSQLのアーキテクチャはMySQLの従来のRDBMS設計と根本的に異なります
bench-sysbench-srv-disc-perf-storage = VibeSQLは分析およびOLTPワークロード向けに最適化されたインメモリカラム型ストレージエンジンを使用し、MySQLのディスクベースのInnoDBページ管理オーバーヘッドを回避します
bench-sysbench-srv-disc-perf-locking = 重い行レベルロックやMVCC記録なし—VibeSQLは最新のマルチコアCPU向けに設計された軽量な同時実行制御を使用します
bench-sysbench-srv-disc-perf-protocol = MySQLのプロトコルと比較して最小限のシリアライゼーションオーバーヘッドを持つ効率的なPostgreSQLワイヤプロトコル実装
bench-sysbench-srv-disc-perf-writes = 書き込み操作（挿入/更新）は最大の性能向上（<strong>8-12x</strong>）を示します。VibeSQLはMySQLのredoログ、undoログ、ダブルライトバッファの同期を回避するためです
bench-sysbench-srv-disc-perf-reads = 読み取り操作はキャッシュ効率の良いカラムアクセスパターンとゼロディスクI/Oにより、小さいながらも一貫した性能向上（<strong>2-3x</strong>）を示します
...
challenge-record-beat = これを5%上回りますか？トロフィーを獲得するには<strong>{ $target }日以下</strong>が必要です。
