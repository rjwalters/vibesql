...
bench-sysbench-srv-disc-mysql = تقارن مقاييس الخادم مع MySQL لتقييم VibeSQL كبديل مباشر لقواعد البيانات التقليدية العميل-الخادم. يتفوق VibeSQL Server على MySQL في جميع عمليات Sysbench، مع تسريعات تتراوح من <strong>2.4x</strong> (استعلامات النطاق) إلى <strong>12.8x</strong> (التحديثات المفهرسة).
bench-sysbench-srv-disc-perf-title = لماذا VibeSQL Server أسرع
bench-sysbench-srv-disc-perf-arch = تختلف بنية VibeSQL جوهرياً عن تصميم RDBMS التقليدي لـ MySQL
bench-sysbench-srv-disc-perf-storage = يستخدم VibeSQL محرك تخزين عمودي في الذاكرة مُحسّن لأحمال العمل التحليلية وOLTP، متجنباً عبء إدارة صفحات InnoDB المستندة إلى القرص
bench-sysbench-srv-disc-perf-locking = لا يوجد قفل ثقيل على مستوى الصف أو محاسبة MVCC—يستخدم VibeSQL تحكماً خفيفاً في التزامن مصمماً لمعالجات متعددة النوى الحديثة
bench-sysbench-srv-disc-perf-protocol = تنفيذ فعّال لبروتوكول PostgreSQL مع حد أدنى من عبء التسلسل مقارنة ببروتوكول MySQL
bench-sysbench-srv-disc-perf-writes = تُظهر عمليات الكتابة (الإدراج/التحديث) أكبر المكاسب (<strong>8-12x</strong>) لأن VibeSQL يتجنب مزامنة سجل إعادة التنفيذ وسجل التراجع ومخزن الكتابة المزدوجة في MySQL
bench-sysbench-srv-disc-perf-reads = تُظهر عمليات القراءة مكاسب أصغر لكن ثابتة (<strong>2-3x</strong>) بفضل أنماط الوصول العمودي الفعّالة للذاكرة المخبئية وعدم وجود I/O على القرص
...
challenge-record-beat = تريد التغلب على هذا بنسبة 5%؟ هذا يعني <strong>{ $target } يوم أو أقل</strong> للمطالبة بالكأس.
