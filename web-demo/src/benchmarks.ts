...
const li = (label: string, value: string): string => `<li><strong>${label}:</strong> ${value}</li>`
...
const methodology = (
  ...
  ${
    notes
      ?.map(
        note => `
  <p class="mt-4 text-gray-500 dark:text-gray-400 text-sm">${note}</p>`
      )
      .join('') ?? ''
  }
`
...
const SUITE_CONFIGS: Record<BenchmarkSuite, SuiteConfig> = {
    ...
    getDiscussion: () =>
      discussion([
        ...
        },
        {
          ...
          content:
            pI18n('bench-tpcds-disc-optimization') +
            bulletsI18n([
    ...
    getMethodology: () =>
      methodology(
        ...
        [
          ...
          li(
            t('bench-operations'),
            'Single-statement operations (no multi-statement transactions)'
          ),
    ...
    getDiscussion: () =>
      discussion([
        ...
        },
        {
          ...
          content: bulletsI18n([
            {
              labelKey: 'bench-sysbench-srv-disc-perf-arch',
              descKey: 'bench-sysbench-srv-disc-perf-storage',
            },
            {
              labelKey: 'bench-bullet-concurrency',
              descKey: 'bench-sysbench-srv-disc-perf-locking',
            },
        ...
        },
        {
          ...
          content: bulletsI18n([
            {
              labelKey: 'bench-bullet-connection-pooling',
              descKey: 'bench-sysbench-srv-disc-pooling',
            },
            { labelKey: 'bench-bullet-stmt-caching', descKey: 'bench-sysbench-srv-disc-caching' },
            {
              labelKey: 'bench-bullet-extended-protocol',
              descKey: 'bench-sysbench-srv-disc-extended',
            },
          ]),
        },
      ]),
  ...
  'tpcc-server': {
    ...
    descriptions: {
      mixed:
        'Mixed Workload - Standard TPC-C transaction mix (45% New-Order, 43% Payment, 4% Order-Status, 4% Delivery, 4% Stock-Level)',
      new_order: 'New Order - Complex transaction with inventory checks and order creation',
      payment: 'Payment - Update customer balance and warehouse/district totals',
      order_status: 'Order Status - Read-only query for customer order history',
      delivery: 'Delivery - Batch processing of pending orders',
    ...
    getMethodology: () =>
      methodology(
        ...
        [
          li(
            t('bench-binary-size'),
            'Size of the compiled native binary in bytes (stripped release build)'
          ),
          li(
            t('bench-startup-time'),
            'Time from process start to first query result (CREATE TABLE, INSERT, SELECT)'
          ),
...
function resetSummaryCardHeaders(): void {
  const sqliteHeader = document
    .querySelector('#avg-speedup-sqlite')
  ...
  const duckdbHeader = document
    .querySelector('#avg-speedup-duckdb')
    ?.parentElement?.querySelector('h3')
...
/**
 * Update both speedup summary cards (vs SQLite and vs DuckDB, or vs MySQL for server mode)
 */
function updateSpeedupSummary(
  ...
  if (isServerMode) {
    ...
    const duckdbCard = document
      .getElementById('avg-speedup-duckdb')
      ?.closest('.bg-white, .dark\\:bg-gray-800') as HTMLElement
    if (duckdbCard) {
      duckdbCard.style.display = 'none'
    }
  } else {
    // Show DuckDB card in embedded mode
    const duckdbCard = document
      .getElementById('avg-speedup-duckdb')
      ?.closest('.bg-white, .dark\\:bg-gray-800') as HTMLElement
    if (duckdbCard) {
      duckdbCard.style.display = ''
    }
  }

  // Update first speedup card (SQLite for embedded, MySQL for server)
  ...
  // Update DuckDB speedup (only for embedded mode)
  if (!isServerMode) {
 ...
 *
 * Server benchmark formats:
 *   tpch_server_q1_vibesql_server -> database: vibesql_server, queryNum: q1
 *   tpch_server_q1_mysql -> database: mysql, queryNum: q1
 *   tpcc_server_new_order_vibesql_server -> database: vibesql_server
 *   sysbench_server_point_select_mysql -> database: mysql
 *
 * Embedded benchmark formats:
 *   tpch_q1_pricing_summary_report_vibesql -> database: vibesql, queryNum: q1
 *   tpcc_new_order_vibesql -> database: vibesql
 */
function parseBenchmarkName(
  name: string,
  suite: BenchmarkSuite
): { operation: string; database: string; queryNum?: string; description?: string } {
  const config = SUITE_CONFIGS[suite]
  const parts = name.split('_')

  // Handle vibesql_server as a compound database name (last two parts)
  let database: string
  let endSlice: number
  if (
    parts.length >= 2 &&
    parts[parts.length - 2] === 'vibesql' &&
    parts[parts.length - 1] === 'server'
  ) {
  ...
  // TPC-H Server format: "tpch_server_q1_vibesql_server" or "tpch_server_q1_mysql"
  if (name.startsWith('tpch_server_')) {
    // parts: ['tpch', 'server', 'q1', 'vibesql', 'server'] or ['tpch', 'server', 'q1', 'mysql']
    const queryNum = parts[2] // e.g., "q1"
    const description = config.descriptions[queryNum]
    const operation = parts.slice(3, endSlice).join('_')
    return { operation, database, queryNum, description }
  }

  // TPC-H Embedded format: "tpch_q1_vibesql" or "tpch_q1_pricing_summary_report_vibesql"
  if (name.startsWith('tpch_')) {
    const queryNum = parts[1] // e.g., "q1"
    const description = config.descriptions[queryNum]
    // Use queryNum as operation for grouping (handles both simple "tpch_q1_vibesql" and
    // extended "tpch_q1_pricing_summary_report_vibesql" formats)
    const operation = queryNum
    return { operation, database, queryNum, description }
  }
  ...
  if (name.startsWith('tpcds_')) {
    const queryNum = parts[1] // e.g., "q1"
    const description = config.descriptions[queryNum]
    // Use queryNum as operation for grouping (since TPC-DS has only query number + database)
    const operation = queryNum
    return { operation, database, queryNum, description }
  }

  // TPC-C Server format: "tpcc_server_new_order_vibesql_server"
  if (name.startsWith('tpcc_server_')) {
    const operation = parts.slice(2, endSlice).join('_')
    const description = config.descriptions[operation]
    return { operation, database, description }
  }

  // TPC-C Embedded format: "tpcc_new_order_vibesql"
  if (name.startsWith('tpcc_')) {
    const operation = parts.slice(1, endSlice).join('_')
    const description = config.descriptions[operation]
    return { operation, database, description }
  }

  // Sysbench Server format: "sysbench_server_point_select_mysql"
  if (name.startsWith('sysbench_server_')) {
    const operation = parts.slice(2, endSlice).join('_')
    const description = config.descriptions[operation]
    return { operation, database, description }
  }

  // Sysbench Embedded format: "sysbench_point_select_vibesql"
 ...
 */
function groupBenchmarksByOperation(
  benchmarks: Benchmark[],
  suite: BenchmarkSuite
): Map<string, Map<string, Benchmark>> {
...
/**
 * Sort entries by query number (Q1, Q2, ..., Q9, Q10, Q11, ... Q99)
 * Handles formats like "q1", "q10", "q99"
 */
function sortEntriesByQueryNumber<T>(entries: [string, T][]): [string, T][] {
  return entries.sort((a, b) => {
    const aMatch = a[0].match(/^q(\d+)$/i)
    const bMatch = b[0].match(/^q(\d+)$/i)
    if (aMatch && bMatch) {
      return parseInt(aMatch[1]) - parseInt(bMatch[1])
    }
    // Fall back to string comparison for non-query operations
...
function renderResultsTable(data: BenchmarkResults, suite: BenchmarkSuite): void {
  ...
  // Sort entries by query number for TPC suites
  const sortedEntries = sortEntriesByQueryNumber([...grouped.entries()])

  for (const [operation, databases] of sortedEntries) {
    ...
    const createTimeCell = (bench: Benchmark | undefined, name: string): HTMLTableCellElement => {
    ...
    // Create cells for each database based on benchmark type
    // Server benchmarks (tpch-server, tpcc-server, sysbench-server) compare vibesql_server vs mysql
    // Embedded benchmarks compare vibesql vs sqlite vs duckdb
    const isServerBenchmark = suite.endsWith('-server')

    if (isServerBenchmark) {
      // Server mode: VibeSQL Server vs MySQL
      const vibesqlServerCell = createTimeCell(vibesqlServer, 'vibesql_server')
      row.appendChild(vibesqlServerCell)

      const mysqlCell = createTimeCell(mysql, 'mysql')
      row.appendChild(mysqlCell)
    } else {
      // Embedded mode: VibeSQL vs SQLite vs DuckDB
    ...
    if (isServerBenchmark) {
      // Server mode: VibeSQL Server vs MySQL
      if (vibesqlServer && vibesqlServer.stats.mean > 0 && mysql && mysql.stats.mean > 0) {
        // Use sqliteSpeedup slot for MySQL comparison in server mode
        sqliteSpeedup.total += calculateSpeedup(vibesqlServer.stats.mean, mysql.stats.mean)
        sqliteSpeedup.count++
      }
    } else {
      // Embedded mode: VibeSQL vs SQLite and DuckDB
  ...
  // Update summary cards (for server mode, sqliteSpeedup contains MySQL comparison)
  updateSpeedupSummary(sqliteSpeedup, duckdbSpeedup, suite.endsWith('-server'))
...
function renderChart(data: BenchmarkResults, suite: BenchmarkSuite): void {
  ...
  for (const [operation, databases] of sortedEntries) {
    ...
    const hasValidData = isServerBenchmark
      ? vibesqlServerValid || mysqlValid
      : vibesqlValid || sqliteValid || duckdbValid
  ...
  // Build datasets array based on benchmark mode
  let datasets
  if (isServerBenchmark) {
    // Server mode: VibeSQL Server vs MySQL
    datasets = [
      createDataset('vibesql_server', vibesqlServerData),
...
function renderFootprintServerTable(data: FootprintResults): void {
  ...
  const metrics = [
    ...
    },
    {
      ...
      note:
        vibesql.wasm_size_gzip_bytes && vibesql.wasm_size_bytes
    ...
    },
    {
      ...
      note:
        vibesql.wasm_size_brotli_bytes && vibesql.wasm_size_bytes
  ...
  for (const metric of metrics) {
    ...
    valueCell.className =
      'px-4 py-3 text-right text-primary-light dark:text-primary-dark font-semibold'
      ...
      datasets: [
        {
          ...
          borderColor: ['rgba(156, 163, 175, 1)', 'rgba(34, 197, 94, 1)', 'rgba(59, 130, 246, 1)'],
          borderWidth: 1,
        },
      ],
    },
...
function renderSysbenchTable(data: BenchmarkResults, suite: BenchmarkSuite): void {
  ...
  for (const [operation, databases] of grouped.entries()) {
    ...
    if (primary && primary.stats.mean > 0) {
      primaryCell.textContent =
        formatTime(primary.stats.mean, primary.stats.stddev) || t('bench-na')
    ...
    if (comparison && comparison.stats.mean > 0) {
      compCell.textContent =
        formatTime(comparison.stats.mean, comparison.stats.stddev) || t('bench-na')
  ...
  // Update summary cards (pass isServer to correctly show MySQL comparison label)
  updateSpeedupSummary(sqliteSpeedup, duckdbSpeedup, isServer)
...
function renderSysbenchChart(data: BenchmarkResults, suite: BenchmarkSuite): void {
  ...
  for (const [operation, databases] of grouped.entries()) {
    ...
    if (primary || comparison) {
      ...
      comparisonData.push(
        comparison && comparison.stats.mean > 0 ? comparison.stats.mean * 1000 : 0
      )
      if (!isServer) {
 ...
 */
function groupTPCCBenchmarksByOperation(
  benchmarks: TPCCBenchmark[]
): Map<string, Map<string, TPCCBenchmark>> {
 ...
 */
function groupTPCCServerBenchmarksByOperation(
  benchmarks: TPCCBenchmark[]
): Map<string, Map<string, TPCCBenchmark>> {
  const grouped = new Map<string, Map<string, TPCCBenchmark>>()
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            label: (context: any) =>
              `${context.dataset.label}: ${(context.parsed.y * 1000).toLocaleString()} TPS`,
          },
        },
      },
    },
...
function renderTPCCServerChart(data: TPCCResults): void {
  ...
  currentChart = new Chart(canvas, {
    ...
    data: {
      labels,
      datasets: [createDataset('vibesql_server', vibesqlData), createDataset('mysql', mysqlData)],
    },
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            label: (context: any) =>
              `${context.dataset.label}: ${context.parsed.y.toLocaleString()} TPS`,
          },
        },
      },
    },
 ...
 */
function groupTPCDSBenchmarksByQuery(
  benchmarks: TPCDSBenchmark[]
): Map<string, Map<string, TPCDSBenchmark>> {
...
function renderTPCDSTable(data: TPCDSResults): void {
  ...
  for (const [queryNum, databases] of sortedQueries) {
    ...
    vibesqlCell.className = vibesqlIsFastest
      ? 'px-4 py-3 text-right font-semibold text-green-600 dark:text-green-400'
      : 'px-4 py-3 text-right text-gray-500 dark:text-gray-400'
    vibesqlCell.textContent =
      vibesql && vibesql.stats.mean > 0
    ...
    sqliteCell.className = sqliteIsFastest
      ? 'px-4 py-3 text-right font-semibold text-green-600 dark:text-green-400'
      : 'px-4 py-3 text-right text-gray-500 dark:text-gray-400'
    sqliteCell.textContent =
      sqlite && sqlite.stats.mean > 0
    ...
    duckdbCell.className = duckdbIsFastest
      ? 'px-4 py-3 text-right font-semibold text-green-600 dark:text-green-400'
      : 'px-4 py-3 text-right text-gray-500 dark:text-gray-400'
    duckdbCell.textContent =
      duckdb && duckdb.stats.mean > 0
...
function renderTPCDSChart(data: TPCDSResults): void {
  ...
  for (const [queryNum, databases] of sortedQueries) {
    ...
    const vibesqlTime =
      vibesql?.stats.status === 'passed' && vibesql.stats.mean > 0 ? vibesql.stats.mean * 1000 : 0
    const sqliteTime =
      sqlite?.stats.status === 'passed' && sqlite.stats.mean > 0 ? sqlite.stats.mean * 1000 : 0
    const duckdbTime =
      duckdb?.stats.status === 'passed' && duckdb.stats.mean > 0 ? duckdb.stats.mean * 1000 : 0
...
function restoreTPCTableHeaders(suite?: BenchmarkSuite): void {
  ...
  const thead = table.querySelector('thead tr')
  if (thead) {
    // Server benchmarks (tpch-server, tpcc-server, sysbench-server) show VibeSQL Server vs MySQL
    if (suite?.endsWith('-server')) {
      thead.innerHTML = `
      ...
      // Embedded benchmarks (tpch, tpcds, tpcc, sysbench-embedded): VibeSQL vs SQLite vs DuckDB
      thead.innerHTML = `
        <th class="px-4 py-3">${t('bench-table-operation')}</th>
        <th class="px-4 py-3 text-right">${t('bench-table-vibesql')}</th>
        <th class="px-4 py-3 text-right">${t('bench-table-sqlite')}</th>
...
async function loadBenchmarkData(suite: BenchmarkSuite): Promise<void> {
  ...
  try {
    ...
    if (suite === 'tpcds') {
      ...
      const hasComparison = data.benchmarks.some(
        b => b.name.endsWith('_sqlite') || b.name.endsWith('_duckdb') || b.name.endsWith('_mysql')
      )
