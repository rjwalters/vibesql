...
**Single-file runs never touch this DB.** `./scripts/tcltest test <file>` (and `make test-tcl-file`) persists its per-test forensics — including `expected_output`/`actual_output` for failed tests — to a **separate sibling DB with the same schema**, `~/.vibesql/test_results/tcl_single_file_results.vbsql`, under its own `run_id` (opt out with `--no-db`, redirect with `--results-db PATH`). This is deliberate: every "latest run" consumer (the canonical queries below, `make test-tcl-status`, `scripts/export_tcl_results.py`, `scripts/tcl_triage.py`) selects `MAX(run_id)` from the canonical DB, so a one-file diagnostic run written there would become the latest run and silently corrupt the canonical pass-rate baseline. To inspect a single-file failure after the fact, query the single-file DB with the same detail-table queries.
...
Run `/repo:release` from this repo to drive a v0.X.Y cut interactively. (This replaced `/loom:release`, which Loom 0.11.0 removed; `/repo:release` discovers and runs this repo's executable `scripts/version.sh` as its bump tool, so the version-bump mechanics below are unchanged.)
