...
fn test_tokenize_where_keyword() {
    ...
    let tokens = lexer.tokenize().unwrap();
    assert_eq!(tokens[0], Token::Keyword { keyword: Keyword::Where, original: "where".to_string() });
}
...
fn test_tokenize_multiple_keywords() {
    ...
    assert_eq!(tokens[2], Token::Keyword { keyword: Keyword::Where, original: "WHERE".to_string() });
