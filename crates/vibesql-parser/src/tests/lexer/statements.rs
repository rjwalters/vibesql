...
fn test_tokenize_select_from_table() {
    ...
    assert_eq!(tokens[2], Token::Keyword { keyword: Keyword::From, original: "FROM".to_string() });
...
fn test_tokenize_select_columns() {
    ...
    assert_eq!(tokens[6], Token::Keyword { keyword: Keyword::From, original: "FROM".to_string() });
...
fn test_tokenize_select_with_where() {
    ...
    assert_eq!(tokens[2], Token::Keyword { keyword: Keyword::From, original: "FROM".to_string() });
    assert_eq!(tokens[3], Token::Identifier("users".to_string()));
    assert_eq!(tokens[4], Token::Keyword { keyword: Keyword::Where, original: "WHERE".to_string() });
