...
pub fn parse_revoke(parser: &mut crate::Parser) -> Result<RevokeStmt, ParseError> {
    ...
    let grant_option_for = if parser.peek_keyword(Keyword::Grant) {
    ...
    let granted_by = if parser.peek_keyword(Keyword::Granted) {
    ...
    let cascade_option = if parser.peek_keyword(Keyword::Cascade) {
