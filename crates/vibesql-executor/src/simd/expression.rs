...
mod tests {
    ...
    #[test]
    fn test_can_use_simd_for_expression() {
        // Simple arithmetic - should use SIMD with enough rows
        let expr = Expression::BinaryOp {
            left: Box::new(Expression::Identifier("price".to_string())),
            op: BinaryOperator::Multiply,
            right: Box::new(Expression::Identifier("quantity".to_string())),
        };
        assert!(can_use_simd_for_expression(&expr, 200));
        assert!(!can_use_simd_for_expression(&expr, 50)); // Too few rows

        // Complex expression - should not use SIMD
    ...
    #[test]
    fn test_is_simple_operand() {
        // Column reference is simple
        assert!(is_simple_operand(&Expression::Identifier(
            "price".to_string()
        )));

        // Literal is simple
        assert!(is_simple_operand(&Expression::Value(Value::Number(
            "100".to_string()
        ))));

        // Nested arithmetic is simple
        let expr = Expression::BinaryOp {
        ...
        // Subquery is NOT simple
        assert!(!is_simple_operand(&Expression::Subquery(Box::new(
            SelectStatement {
                select: vec![],
                from: None,
                where_clause: None,
