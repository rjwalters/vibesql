...
mod eliminate_unused_tables_tests {
    ...
    fn make_column_ref(table: Option<&str>, column: &str) -> Expression {
        match table {
            Some(t) => Expression::ColumnRef(vibesql_ast::ColumnIdentifier::qualified(
                t, false, column, false,
            )),
...
mod helper_function_tests {
    ...
    fn has_unqualified_column_ref_detects_unqualified() {
        let qualified = Expression::ColumnRef(vibesql_ast::ColumnIdentifier::qualified(
            "t1", false, "col1", false,
        ));
        assert!(!has_unqualified_column_ref(&qualified));

        let unqualified =
            Expression::ColumnRef(vibesql_ast::ColumnIdentifier::simple("col1", false));
