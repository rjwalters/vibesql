...
fn create_test_schema() -> CombinedSchema {
    let table_schema = TableSchema::new(
        "test".to_string(),
        vec![
            ColumnSchema::new("day_name".to_string(), DataType::Varchar { max_length: Some(20) }, false),
...
fn create_simple_case_expression(col_name: &str, match_value: SqlValue, result_col: &str) -> Expression {
...
fn test_compile_when_equals_then_column() {
    ...
    match compiled.unwrap() {
        CompiledCaseExpression::WhenEqualsThenColumn {
            ...
            assert_eq!(condition_value, SqlValue::Varchar("Sunday".to_string()));
    ...
    let expr = Expression::Case {
        ...
        when_clauses: vec![CaseWhen {
            conditions: vec![Expression::BinaryOp {
                ...
                op: BinaryOperator::Equal,
                right: Box::new(Expression::Literal(SqlValue::Varchar("Sunday".to_string()))),
            }],
...
fn test_does_not_compile_simple_case_with_operand() {
    ...
    let expr = Expression::Case {
        ...
        when_clauses: vec![CaseWhen {
            conditions: vec![Expression::Literal(SqlValue::Varchar("Sunday".to_string()))],
            result: Expression::ColumnRef {
                table: None,
        ...
        when_clauses: vec![
            ...
            CaseWhen {
                conditions: vec![Expression::BinaryOp {
                    ...
                    op: BinaryOperator::Equal,
                    right: Box::new(Expression::Literal(SqlValue::Varchar("Monday".to_string()))),
                }],
...
fn test_literal_on_left_side_of_equality() {
    ...
    let expr = Expression::Case {
        ...
        when_clauses: vec![CaseWhen {
            conditions: vec![Expression::BinaryOp {
                left: Box::new(Expression::Literal(SqlValue::Varchar("Sunday".to_string()))),
                op: BinaryOperator::Equal,
