...
fn test_update_invalidates_prewarmed_cache() {
    ...
    let stmt = UpdateStmt {
        ...
        where_clause: Some(WhereClause::Condition(Expression::BinaryOp {
            ...
            op: BinaryOperator::Equal,
            right: Box::new(Expression::Literal(SqlValue::Varchar("Bob".to_string()))),
        })),
    };
    let count = UpdateExecutor::execute(&stmt, &mut db).unwrap();
    assert_eq!(count, 1, "Should update 1 row");
...
fn test_update_multiple_columns_invalidates_cache() {
    ...
    let stmt = UpdateStmt {
        ...
        assignments: vec![
            ...
            Assignment {
                ...
                value: Expression::Literal(SqlValue::Varchar("Management".to_string())),
            },
        ],
        where_clause: Some(WhereClause::Condition(Expression::BinaryOp {
            left: Box::new(Expression::ColumnRef { table: None, column: "id".to_string() }),
    ...
    assert!(
        (0..3).any(|i| matches!(dept_col.get(i), SqlValue::Varchar(ref s) if s == "Management")),
