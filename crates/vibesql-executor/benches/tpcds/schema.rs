...
pub fn load_vibesql(scale_factor: f64) -> VibeDB {
    ...
    // Enable case-insensitive identifier lookups for SQL standard compliance.
    // The TPC-DS schema uses lowercase table/column names, but the SQL parser
...
fn load_date_dim_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for d_date_sk in 1..=num_dates {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(d_date_id)),
            SqlValue::Date(
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(day_names[d_dow as usize])),
            SqlValue::Varchar(arcstr::ArcStr::from(quarter_name)),
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_holiday
            SqlValue::Varchar(arcstr::ArcStr::from(if is_weekend { "Y" } else { "N" })),
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_following_holiday
            ...
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_current_day
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_current_week
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_current_month
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_current_quarter
            SqlValue::Varchar(arcstr::ArcStr::from("N")), // d_current_year
        ]);
        rows.push(row);
...
fn load_time_dim_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 0..num_times {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(t_time_id)),
            SqlValue::Integer(t_time_sk as i64),
            SqlValue::Integer(hour as i64),
            SqlValue::Integer(minute as i64),
            SqlValue::Integer(second as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(am_pm)),
            SqlValue::Varchar(arcstr::ArcStr::from(shift)),
            SqlValue::Varchar(arcstr::ArcStr::from(sub_shift)),
            SqlValue::Varchar(arcstr::ArcStr::from(meal_time)),
        ]);
        rows.push(row);
    }
...
fn load_item_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.item_count {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(BRANDS[brand_idx])),
            SqlValue::Integer((class_idx + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(CLASSES[class_idx])),
            SqlValue::Integer((category_idx + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(CATEGORIES[category_idx])),
            SqlValue::Integer(((i % 10) + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Manufacturer#{}", (i % 10) + 1))),
            SqlValue::Varchar(arcstr::ArcStr::from(ITEM_SIZES[size_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("formula{}", i % 20))),
            SqlValue::Varchar(arcstr::ArcStr::from(ITEM_COLORS[color_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from("Each")),
            SqlValue::Varchar(arcstr::ArcStr::from("Unknown")),
...
fn load_customer_address_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.customer_address_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(ca_address_id)),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{}", data.random_i32(1, 999)))),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_varchar(30))),
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{} County", STATES[state_idx]))),
            SqlValue::Varchar(arcstr::ArcStr::from(STATES[state_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_zip())),
            SqlValue::Varchar(arcstr::ArcStr::from("United States")),
            SqlValue::Numeric(-5.0 + (state_idx as f64 * 0.1)), // ca_gmt_offset
            SqlValue::Varchar(arcstr::ArcStr::from("residential")),
        ]);
        rows.push(row);
...
fn load_customer_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.customer_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(c_customer_id)),
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(salutations[sal_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("FirstName{}", i % 1000))),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("LastName{}", i % 2000))),
            SqlValue::Varchar(arcstr::ArcStr::from(if i % 3 == 0 { "Y" } else { "N" })),
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_email())),
...
fn load_store_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.store_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(s_store_id)),
            SqlValue::Date(Date::from_str("1998-01-01").unwrap()),
            SqlValue::Null, // s_rec_end_date
            SqlValue::Null, // s_closed_date_sk
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Store#{}", i))),
            SqlValue::Integer(data.random_i32(50, 500) as i64),
            SqlValue::Integer(data.random_i32(5000, 50000) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from("8AM-10PM")),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Manager{}", i % 100))),
            SqlValue::Integer((i % 10 + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from("Unknown")),
            SqlValue::Varchar(arcstr::ArcStr::from("Market description")),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("MarketManager{}", i % 50))),
            SqlValue::Integer((i % 5 + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Division{}", i % 5 + 1))),
            SqlValue::Integer((i % 3 + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Company{}", i % 3 + 1))),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{}", data.random_i32(1, 999)))),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_varchar(30))),
            SqlValue::Varchar(arcstr::ArcStr::from("Avenue")),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Suite {}", data.random_i32(100, 999)))),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_city())),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{} County", STATES[state_idx]))),
            SqlValue::Varchar(arcstr::ArcStr::from(STATES[state_idx])),
...
fn load_promotion_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.promotion_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(p_promo_id)),
...
fn load_ship_mode_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(sm_ship_mode_id)),
            SqlValue::Varchar(arcstr::ArcStr::from(types[type_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(SHIP_MODES[mode_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(carriers[carrier_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Contract{}", i))),
        ]);
        rows.push(row);
    }
...
fn load_reason_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(r_reason_id)),
            SqlValue::Varchar(arcstr::ArcStr::from(REASONS[i - 1])),
        ]);
        rows.push(row);
    }
...
fn load_catalog_page_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.catalog_page_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(cp_catalog_page_id)),
            SqlValue::Integer(start_date_sk as i64),
            SqlValue::Integer(end_date_sk as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(departments[dept_idx])),
            SqlValue::Integer(((i / 100) + 1) as i64),
            SqlValue::Integer(((i % 100) + 1) as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Catalog page {} description", i))),
...
fn load_web_site_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.web_site_count {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(arcstr::ArcStr::from(city)),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{} County", STATES[state_idx]))),
            SqlValue::Varchar(arcstr::ArcStr::from(STATES[state_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(zip)),
            SqlValue::Varchar(arcstr::ArcStr::from("United States")),
                ...
                for &credit in CREDIT_RATINGS.iter() {
                    for &dep_count in &DEP_COUNTS[0..3] {
                        ...
                        let row = Row::new(vec![
                            ...
                            SqlValue::Varchar(arcstr::ArcStr::from(*gender)),
                            SqlValue::Varchar(arcstr::ArcStr::from(*marital)),
                            SqlValue::Varchar(arcstr::ArcStr::from(*education)),
                            SqlValue::Integer(purchase_estimate as i64),
                            SqlValue::Varchar(arcstr::ArcStr::from(credit)),
            ...
            for &dep_count in DEP_COUNTS.iter() {
                for &vehicle_count in VEHICLE_COUNTS.iter() {
                    ...
                    let row = Row::new(vec![
                        ...
                        SqlValue::Varchar(arcstr::ArcStr::from(buy_potential)),
                        SqlValue::Integer(dep_count as i64),
...
fn load_call_center_vibesql(db: &mut VibeDB, data: &mut TPCDSData) {
    ...
    for i in 1..=data.call_center_count {
        ...
        let row = Row::new(vec![
            SqlValue::Integer(i as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(cc_call_center_id)),
            SqlValue::Null, // cc_rec_start_date
            SqlValue::Null, // cc_rec_end_date
            SqlValue::Null, // cc_closed_date_sk
            SqlValue::Integer(open_date_sk as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Call Center {}", i))),
            SqlValue::Varchar(arcstr::ArcStr::from(classes[class_idx])),
            SqlValue::Integer(employees as i64),
            SqlValue::Integer(sq_ft as i64),
            SqlValue::Varchar(arcstr::ArcStr::from(hours[hours_idx])),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_varchar(30))),
            SqlValue::Integer((i % 6 + 1) as i64), // cc_mkt_id
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Market Class {}", i % 10))),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Market Description {}", i))),
            SqlValue::Varchar(arcstr::ArcStr::from(data.random_varchar(30))),
            SqlValue::Integer((i % 3 + 1) as i64), // cc_division
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Division {}", i % 3 + 1))),
            SqlValue::Integer((i % 2 + 1) as i64), // cc_company
            SqlValue::Varchar(arcstr::ArcStr::from(format!("Company {}", i % 2 + 1))),
            SqlValue::Varchar(arcstr::ArcStr::from(format!("{}", data.random_i32(100, 9999)))),
