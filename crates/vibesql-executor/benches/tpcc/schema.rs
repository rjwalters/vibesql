...
fn load_item_vibesql(db: &mut VibeDB, data: &mut TPCCData) {
    ...
    for i_id in 1..=num_items {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(std::sync::Arc::from(item.i_name)),
            SqlValue::Numeric(item.i_price),
            SqlValue::Varchar(std::sync::Arc::from(item.i_data)),
        ]);
...
fn load_warehouse_vibesql(db: &mut VibeDB, data: &mut TPCCData, w_id: i32) {
    ...
    let row = Row::new(vec![
        ...
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_name)),
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_street_1)),
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_street_2)),
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_city)),
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_state)),
        SqlValue::Varchar(std::sync::Arc::from(warehouse.w_zip)),
...
fn load_stock_vibesql(db: &mut VibeDB, data: &mut TPCCData, w_id: i32) {
    ...
    for i_id in 1..=num_items {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_01)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_02)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_03)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_04)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_05)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_06)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_07)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_08)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_09)),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_dist_10)),
            SqlValue::Integer(stock.s_ytd as i64),
            SqlValue::Integer(stock.s_order_cnt as i64),
            SqlValue::Integer(stock.s_remote_cnt as i64),
            SqlValue::Varchar(std::sync::Arc::from(stock.s_data)),
        ]);
...
fn load_district_vibesql(db: &mut VibeDB, data: &mut TPCCData, d_id: i32, w_id: i32) {
    ...
    let row = Row::new(vec![
        ...
        SqlValue::Varchar(std::sync::Arc::from(district.d_name)),
        SqlValue::Varchar(std::sync::Arc::from(district.d_street_1)),
        SqlValue::Varchar(std::sync::Arc::from(district.d_street_2)),
        SqlValue::Varchar(std::sync::Arc::from(district.d_city)),
        SqlValue::Varchar(std::sync::Arc::from(district.d_state)),
        SqlValue::Varchar(std::sync::Arc::from(district.d_zip)),
...
fn load_customer_vibesql(db: &mut VibeDB, data: &mut TPCCData, d_id: i32, w_id: i32) {
    ...
    for c_id in 1..=customers_per_district {
        ...
        let row = Row::new(vec![
            ...
            SqlValue::Varchar(std::sync::Arc::from(customer.c_first)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_middle)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_last)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_street_1)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_street_2)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_city)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_state)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_zip)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_phone)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_since)),
            SqlValue::Varchar(std::sync::Arc::from(customer.c_credit)),
            ...
            SqlValue::Varchar(std::sync::Arc::from(customer.c_data)),
        ]);
        ...
        let history_row = Row::new(vec![
            ...
            SqlValue::Varchar(std::sync::Arc::from(history.h_date)),
            SqlValue::Numeric(history.h_amount),
            SqlValue::Varchar(std::sync::Arc::from(history.h_data)),
        ]);
...
fn load_orders_vibesql(db: &mut VibeDB, data: &mut TPCCData, d_id: i32, w_id: i32) {
    ...
    for o_id in 1..=orders_per_district {
        ...
        let order_row = Row::new(vec![
            ...
            SqlValue::Varchar(std::sync::Arc::from(order.o_entry_d)),
        ...
        for ol_number in 1..=order.o_ol_cnt {
            ...
            let ol_row = Row::new(vec![
                ...
                ol.ol_delivery_d.map(|s| SqlValue::Varchar(std::sync::Arc::from(s))).unwrap_or(SqlValue::Null),
                SqlValue::Integer(ol.ol_quantity as i64),
                SqlValue::Numeric(ol.ol_amount),
                SqlValue::Varchar(std::sync::Arc::from(ol.ol_dist_info)),
            ]);
