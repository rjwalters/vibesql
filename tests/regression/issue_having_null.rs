...
fn test_having_null_in_list() {
    ...
    let rows = select_rows(
        &db,
        "SELECT 1 AS x FROM tab2 GROUP BY col2 HAVING NULL IN (1, 2, 3)"
    );
...
fn test_having_between_null() {
    ...
    let rows = select_rows(
        &db,
        "SELECT col0 FROM tab2 GROUP BY col0 HAVING col0 BETWEEN NULL AND 100"
    );
    ...
    let rows = select_rows(
        &db,
        "SELECT col0 FROM tab2 GROUP BY col0 HAVING col0 BETWEEN 0 AND NULL"
    );
...
fn test_having_in_with_null_in_list() {
    ...
    let rows = select_rows(
        &db,
        "SELECT col0 FROM tab2 GROUP BY col0 HAVING col0 IN (91, NULL)"
    );
    ...
    let rows = select_rows(
        &db,
        "SELECT col0 FROM tab2 GROUP BY col0 HAVING col0 IN (999, NULL)"
    );
...
fn test_having_null_literal() {
    ...
    let rows = select_rows(
        &db,
        "SELECT col0 FROM tab2 GROUP BY col0 HAVING NULL"
    );
