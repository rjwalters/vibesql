    ...
    TableSchema::new(
        ...
        columns
            ...
            .map(|(col_name, data_type)| {
                ColumnSchema::new(col_name.to_uppercase(), data_type, true)
            })
            .collect(),
    )
}
...
fn test_aggregate_with_unary_operators() {
    ...
    let schema = create_table_schema(
        "tab1",
        vec![("col0", DataType::Integer), ("col1", DataType::Integer), ("col2", DataType::Integer)],
    );
    db.create_table(schema).unwrap();

    db.insert_row(
        "TAB1",
        Row::new(vec![SqlValue::Integer(51), SqlValue::Integer(14), SqlValue::Integer(96)]),
    )
    .unwrap();
    db.insert_row(
        "TAB1",
        Row::new(vec![SqlValue::Integer(85), SqlValue::Integer(5), SqlValue::Integer(59)]),
    )
    .unwrap();
    db.insert_row(
        "TAB1",
        Row::new(vec![SqlValue::Integer(91), SqlValue::Integer(47), SqlValue::Integer(68)]),
    )
    .unwrap();
...
fn test_aggregate_with_arithmetic_in_select() {
    ...
    db.insert_row(
        "TAB0",
        Row::new(vec![SqlValue::Integer(97), SqlValue::Integer(1), SqlValue::Integer(99)]),
    )
    .unwrap();
    db.insert_row(
        "TAB0",
        Row::new(vec![SqlValue::Integer(15), SqlValue::Integer(81), SqlValue::Integer(47)]),
    )
    .unwrap();
    db.insert_row(
        "TAB0",
        Row::new(vec![SqlValue::Integer(87), SqlValue::Integer(21), SqlValue::Integer(10)]),
    )
    .unwrap();
...
fn test_aggregate_with_complex_where() {
    ...
    // Setup
    let schema =
        create_table_schema("tab0", vec![("col0", DataType::Integer), ("col1", DataType::Integer)]);
    db.create_table(schema).unwrap();
